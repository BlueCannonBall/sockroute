        ...
        if LOG_PACKETS:
            with open(PACKET_LOG_LOCATION, 'a') as packet_log_file:
                packet_log_file.write(
                    f"[PACKET] server -> client: {list(data)}\n")
        ss.sendall(data)
...
def listen_on_ss(ss, s):
    while True:
        ...
        if LOG_PACKETS:
            with open(PACKET_LOG_LOCATION, 'a') as packet_log_file:
                packet_log_file.write(
                    f"[PACKET] client -> server: {list(data)}\n")
        s.sendall(data)
...
LOG_PACKETS = False
PACKET_LOG_LOCATION = "packets.log"
...
with socket.socket(socket.AF_INET, socket.SOCK_STREAM) as ss:
    ...
    while True:
        ...
        s_thread = threading.Thread(target=listen_on_s,
                                    args=(conn, s),
                                    daemon=True)
        ss_thread = threading.Thread(target=listen_on_ss,
                                     args=(conn, s),
                                     daemon=True)
        s_thread.start()
        ss_thread.start()
